"""
Dashboard Context Service - Context loaders espec├¡ficos por rol y p├ígina.

Extra├¡do de dashboard_service.py para separar responsabilidades.
"""
...
class DashboardContextService:
    ...
    def validate(operation: str, params: dict) -> None:
        if operation == 'get_estudiante_context':
            if params.get('user') is None:
                raise ValueError('Par├ímetro requerido: user')
            if params.get('pagina_solicitada') is None:
                raise ValueError('Par├ímetro requerido: pagina_solicitada')
            if params.get('escuela_rbd') is None:
                raise ValueError('Par├ímetro requerido: escuela_rbd')
            return
        if operation == 'get_asistencia_context':
            if params.get('user') is None:
                raise ValueError('Par├ímetro requerido: user')
            if params.get('colegio') is None:
                raise ValueError('Par├ímetro requerido: colegio')
            return
    ...
    def _execute_get_estudiante_context(params: dict):
        """Get context specific for estudiante role - sin decorador, validaci├│n manual"""
        user = params['user']
    ...
    def _get_estudiante_inicio_context(user, escuela_rbd):
        ...
        try:
            ...
            tareas_pendientes = 0
            if curso_actual:
                # Contar tareas activas y p├║blicas del curso que no han sido entregadas por el estudiante
            ...
            if curso_actual:
                ...
                for idx, clase in enumerate(clases):
                    ...
                    if hasattr(clase, 'total_evaluaciones') and clase.total_evaluaciones > 0:
                        ...
                        if progreso < 20:
                            progreso = 20  # M├¡nimo 20% para que se vea la barra
    ...
    def _get_estudiante_asistencia_context(user, request_get_params):
        ...
        # Porcentaje de asistencia (evitar divisi├│n por 0)
        ...
        return {
            ...
            'clases': clases,
            'sin_datos_asistencia': total_asistencias == 0,  # Flag expl├¡cito
        }
    ...
    def _get_estudiante_clases_context(user):
        ...
        logger.info(f"Estudiante {user.email}: encontradas {len(clases)} clases v├¡a ClaseEstudiante")
        ...
        for idx, clase in enumerate(clases):
            ...
            if hasattr(clase, 'total_evaluaciones') and clase.total_evaluaciones > 0:
                ...
                progreso = min(int((calificaciones_estudiante / clase.total_evaluaciones) * 100), 100)
                if progreso < 15:
                    progreso = 15  # M├¡nimo para visibilidad
    ...
    def _execute_get_asistencia_context(params: dict):
        ...
        # Selecci├│n autom├ítica si no hay filtro
    ...
    def _get_profesor_notas_context(request_get_params, user, colegio):
        ...
        if filtro_clase_id:
            try:
                ...
                # Obtener evaluaciones de la clase con estad├¡sticas
                evaluaciones_qs = Evaluacion.objects.filter(
                    clase=clase_seleccionada,
                    activa=True
                ).order_by('fecha_evaluacion')

                # Calcular estad├¡sticas para cada evaluaci├│n
                evaluaciones = []
                for evaluacion in evaluaciones_qs:
                    ...
                    # Agregar atributos calculados a la evaluaci├│n
