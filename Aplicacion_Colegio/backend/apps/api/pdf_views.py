...
from backend.apps.accounts.models import User, RelacionApoderadoEstudiante, PerfilEstudiante
...
def _can_access_student_api(request_user, estudiante: User) -> bool:
    ...
    if request_user.id == estudiante.id:
        return True

    # Check if request_user is an active guardian of the student
    if RelacionApoderadoEstudiante.objects.filter(
        apoderado__user=request_user,
...
def api_descargar_certificado_notas(request, estudiante_id: int):
    """Genera un PDF de certificado de notas autenticado por JWT."""
    try:
        estudiante = ORMAccessService.get(User, id=estudiante_id)
    except Exception:
        return HttpResponseForbidden('Estudiante no encontrado')

    if not _can_access_student_api(request.user, estudiante):
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def api_descargar_certificado_matricula(request, estudiante_id: int):
    """Genera un PDF de certificado de matrícula autenticado por JWT."""
    try:
        estudiante = ORMAccessService.get(User, id=estudiante_id)
    except Exception:
        return HttpResponseForbidden('Estudiante no encontrado')
...
@api_view(["GET"])
@permission_classes([IsAuthenticated])
def api_descargar_informe_rendimiento(request, estudiante_id: int):
    """Genera un PDF de informe de rendimiento autenticado por JWT."""
    try:
        estudiante = ORMAccessService.get(User, id=estudiante_id)
    except Exception:
        return HttpResponseForbidden('Estudiante no encontrado')
